"""
dashboard/server.py — INDRA Local HTTP Dashboard
...
def _make_uploads_dir() -> Path:
    ...
    for candidate in [
        Path.home() / "Downloads" / "INDRA Uploads",
        Path.home() / "Documents" / "INDRA Uploads",
        BASE_DIR / "uploads",
    ]:
        try:
...
def _ensure_network_access(port: int) -> None:
    ...
    if sys.platform == "win32":
        ...
        port_rule = f"INDRA Dashboard Port {port}"
        prog_rule = "INDRA Dashboard Python"
        ...
        try:
            ...
            if int(ret) > 32:
                ...
                print(
                    "[Dashboard] Phone connections may fail until INDRA is run as Administrator."
                )
...
class DashboardServer:
    ...
    def _ssl_enabled() -> bool:
        ...
        return (certs / "INDRA.key").exists() and (certs / "INDRA.crt").exists()
...
<p>Press <strong style="color:#dde3ed">Remote Control</strong> in INDRA to get a new QR code.</p>
</div></body></html>""")
...
<body>
<script>
  sessionStorage.setItem('INDRA_token','{tok}');
  sessionStorage.setItem('INDRA_key','{key}');
  localStorage.setItem('INDRA_device_token','{dev_tok}');
  setTimeout(function(){{location.replace('/')}},400);
</script>
<p>Connecting to INDRA…</p>
</body></html>""")
    ...
    async def _serve_alias(self) -> None:
        ...
        """
        ssl_key = BASE_DIR / "config" / "certs" / "INDRA.key"
        ssl_cert = BASE_DIR / "config" / "certs" / "INDRA.crt"
    ...
    async def serve(self) -> None:
        ...
        print(f"[Dashboard] Local URL: {proto}://{self._ip}:{PORT}")
        
        try:
            from pyngrok import ngrok
            import requests
            print("[Dashboard] Starting global Ngrok tunnel...")
        ...
        print("[Dashboard] Press 'Remote Control' in INDRA UI to get the QR code.")
        await uvicorn.Server(cfg).serve()
